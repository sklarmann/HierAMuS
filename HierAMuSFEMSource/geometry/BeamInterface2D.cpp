...
auto BeamInterface2D::getLocalJacobian(PointerCollection &pointers,
  ...
  auto &edge = pointers.getGeometryData()->getEdge(this->edges[localEdgeNumber]);

  auto &V1 = edge.getVertex(pointers, 0);
  auto &V2 = edge.getVertex(pointers, 1);
...
auto BeamInterface2D::getCoordinates(PointerCollection &pointers,
  ...
  coor.setZero();
  auto &Edge =
      pointers.getGeometryData()->getEdge(this->edges[IntPoint.sectionNumber]);
  IntegrationPoint eInt;
  eInt.xi = IntPoint.eta;
  coor = Edge.getCoordinates(pointers, eInt);
...
void BeamInterface2D::getLocalWarpingShapesA1(
  ...
  auto &ledge = pointers.getGeometryData()->getEdge(this->edges[localEdgeNumber]);

  Types::Vector3<prec> edgeDir = ledge.getDirectionVector(pointers);
  ...
  ledge.getH1Shapes(pointers, this->warpingOrder, eshape, eshapeDeriv, eta);
  eshapeDeriv /= jacobi;
...
void BeamInterface2D::getLocalH1ShapesA2(PointerCollection &pointers,
  ...
  Types::VectorX<prec> eshape;
  Types::VectorX<prec> eshapeDeriv;
  edge.getH1Shapes(pointers, shapeOrder, eshape, eshapeDeriv, eta);

  prec dl = this->getLocalJacobian(pointers, localEdgeNumber, eta);
  eshapeDeriv /= dl;

  std::vector<GenericNodes *> tnodes;
  edge.getNodes(pointers, tnodes, meshId);
  ...
  indexType numShapes;
  tnodes.size() < shapeOrder + 1 ? numShapes = tnodes.size()
                                 : numShapes = shapeOrder + 1;

  indexType numVerts = edge.getNumberOfVerts();

  for (auto i = 0; i < numVerts; ++i) {
    auto &tVert = edge.getVertex(pointers, i);
...
void BeamInterface2D::getH1ShapesA1(PointerCollection &pointers,
  ...
  tedge.getH1Shapes(pointers, order, shapes, shapeDerivative, xi);

  auto &Vert = pointers.getGeometryData()->getVertex(this->beamNode);
...
void BeamInterface2D::computeGeometry(PointerCollection &pointers) {
  ...
  coorA = edge.getCoordinates(pointers, prec(-1));
  coorB = edge.getCoordinates(pointers, prec(1));
...
auto BeamInterface2D::getCrossSectionPosition(PointerCollection &pointers,
    ...
    -> prec {
  auto &edge = pointers.getGeometryData()->getEdge(this->edges[edgenum]);
  Types::Vector3<prec> coor = edge.getCoordinates(pointers, eta);
...
auto BeamInterface2D::getDA(PointerCollection &pointers,
  ...
  auto &tedge = pointers.getGeometryData()->getEdge(this->edges[localEdgeNumber]);
  auto &V1 = tedge.getVertex(pointers, 0);
  auto &V2 = tedge.getVertex(pointers, 1);
  auto c2 = V2.getCoordinates();
  auto c1 = V1.getCoordinates();
  auto dist = c2-c1;
  prec temp;
...
void BeamInterface2D::setH1ShapesSurface(PointerCollection &pointers,
  ...
  for (auto &i : this->edges) {
    auto &edge = pointers.getGeometryData()->getEdge(i);
    edge.setH1Shapes(pointers, meshId, order, type);
  }
}
...
void BeamInterface2D::setDofBeamNode(PointerCollection &pointers,
  ...
  auto &vert = pointers.getGeometryData()->getVertex(this->beamNode);
  vert.setNodeSet(pointers, meshId, 1, type);
}
...
void BeamInterface2D::getDofsOnSolid(PointerCollection &pointers,
  ...
  for (auto &i : this->edges) {
    ...
    for (auto j = 0; j < nVerts; ++j) {
      auto &Vert = tedge.getVertex(pointers, j);
      indexType idd = Vert.getId();
      idd = this->globalLocalVertIdMap[idd];
      idd *= 3;
...
void BeamInterface2D::getNodesOnSolid(PointerCollection &pointers,
  ...
  for (auto &i : this->edges) {
    auto &edge = pointers.getGeometryData()->getEdge(i);
    tempNodes.clear();
    edge.getNodes(pointers, tempNodes, meshId);
...
void BeamInterface2D::getDofsOnBeamNode(PointerCollection &pointers,
                                        ...
                                        indexType meshId) {
  auto &Vert = pointers.getGeometryData()->getVertex(this->beamNode);
  Vert.getNodeEquationIds(pointers, Dofs, meshId, 0);
}
...
void BeamInterface2D::getAllDofs(PointerCollection &pointers,
  ...
  for (auto &ee : this->edges) {
    auto &edge = pointers.getGeometryData()->getEdge(ee);
    for (auto j = 0; j < edge.getNumberOfVerts(); ++j) {
      auto &vert = edge.getVertex(pointers, j);
      vert.getAllEquationsIds(pointers, tempDofs);
      Dofs.insert(Dofs.end(), tempDofs.begin(), tempDofs.end());
      tempDofs.clear();
    }
    edge.getAllEquationsIds(pointers, tempDofs);
    Dofs.insert(Dofs.end(), tempDofs.begin(), tempDofs.end());
    tempDofs.clear();
  }
  auto &vert = pointers.getGeometryData()->getVertex(this->beamNode);
...
void BeamInterface2D::computeAlphaBetaParameters(PointerCollection &pointers) {
  ...
  GP.setTypeOrder(IntegrationType::Gauss1D, this->warpingOrder * 2);
  ...
  for (auto edgeNum = 0; edgeNum < this->edges.size(); ++edgeNum) {
    ...
    for (auto ngp = 0; ngp < GP.getTotalGP(); ++ngp) {
      ...
      edge.getH1Shapes(pointers, this->warpingOrder, shape, shapeDerivative,
                        GP.getXi(ngp));
      prec jacobi = edge.getJacobian(pointers, GP.getXi(ngp));
      indexType id;
      indexType totVerts = edge.getNumberOfVerts();
      for (auto nvert = 0; nvert < totVerts; ++nvert) {
        id = edge.getVertex(pointers, nvert).getId(); // get global id
...
void BeamInterface2D::setBCOnVert(PointerCollection &pointers,
  ...
  NodeSet *tempSet;
  tempSet = Vert.getSetMeshId(pointers, meshID);
  tempSet->getNodes(pointers, Nodes);
  GenericNodes *tnode;
  tnode = Nodes[0];
