...
class LinearTriangle : public Faces {
  using ptrCol = PointerCollection;

public:
  LinearTriangle();
  ~LinearTriangle() override;
   ...
   *
   * @return indexType, number of vertices of the element, here 3
   */
  auto getNumberOfVerts() -> indexType override { return 3; };
  /**
   * @brief Returns the number of edges of the element.
   * @return Number of edges as an indexType
   */
  auto getNumberOfEdges() -> indexType override { return 3; };

  /**
   * @brief Set the Vertex numbers of the object.
   *
   * @param[in] vertsIn, vector of 3 integers of type indexType refering to the
   * associated vertex numbers.
   */
  void setEdges(const std::vector<indexType> &edgesIn) override;

  /**
   * @brief Set the Edges of the object. In total the element has 3 edges.
   *
   * @param[in] edgesIn, vector of 3 integers of type indexType refering to the
   ...
   *
   * @param[out] vertsOut, vector of 3 integers of type indexType with global
   ...
   */
  void getVerts(ptrCol &pointers,
                std::vector<Base *> &vertsOut) override;

  /**
   * @brief Get a pointer to the local vertex local_number of the triangle
   * face.
   *
   ...
   */
  auto getVertex(ptrCol &pointers, indexType local_number)
      -> Geometry::Vertex * override;

  /**
   * @brief Get a pointer to the local edge local_number of the triangle
   * face.
   *
   * @param pointers object containtig the pointers to global data.
   ...
   */
  auto getEdge(ptrCol &pointers, indexType local_number)
   ...
   * @param pointers, object containtig the pointers to global data.
   */
  void print(ptrCol &pointers) override;
...
private:
  std::array<indexType, 3> verts, edges;
