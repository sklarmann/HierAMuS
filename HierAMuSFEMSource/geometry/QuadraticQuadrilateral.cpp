...
QuadraticQuadrilateral::QuadraticQuadrilateral()
    : LinearQuadrilateral(), verts_quad({-1, -1, -1, -1, -1}){};
...
void QuadraticQuadrilateral::setVerts(GeometryData &geoData,
                                      std::vector<indexType> &vertsIn) {
  if (vertsIn.size() == 9) {
    std::vector<indexType> lquadverts = {vertsIn[0], vertsIn[1], vertsIn[2],
                                         vertsIn[3]};
    // LinearQuadrilateral::setVerts(...)
    LinearQuadrilateral::setVerts(geoData, lquadverts);
    for (auto i = 4; i < 9; ++i) {
      this->verts_quad[i - 4] = vertsIn[i];
      auto &V1 = geoData.getVertex(vertsIn[i]);
      V1.connectFace(this->id);
    }
  } else {
    std::stringstream temp;
    temp << "Quadratic Quadrilateral geometry element has exactly 9 vertices. "
            "Cannot "
...
void QuadraticQuadrilateral::getVerts(std::vector<indexType> &vertsOut) {
  vertsOut.resize(9);
  for (auto i = 0; i < 4; ++i) {
    vertsOut[i] = this->verts[i];
  }
  for (auto i = 4; i < 9; ++i) {
    vertsOut[i] = this->verts_quad[i];
  }
}

void QuadraticQuadrilateral::getVerts(PointerCollection &pointers,
                                      std::vector<Base *> &vertsOut) {
  vertsOut.clear();
  vertsOut.resize(9);
  for (auto i = 0; i < 4; i++) {
    vertsOut[i] = &pointers.getGeometryData()->getVertex(this->verts[i]);
  }
  for (auto i = 4; i < 9; i++) {
    vertsOut[i] =
        &pointers.getGeometryData()->getVertex(this->verts_quad[i - 4]);
  }
}

auto QuadraticQuadrilateral::getVertex(PointerCollection &pointers,
                                       indexType local_number)
    -> Geometry::Vertex * {
  if (local_number > 8) {
    throw std::runtime_error("Requested a non-existing vertex from a "
                             "quadratic quadrilateral element.");
  }
  if (local_number < 4) {
    return &pointers.getGeometryData()->getVertex(this->verts[local_number]);
  } else {
    return &pointers.getGeometryData()->getVertex(
        this->verts_quad[local_number - 4]);
  }
}
...
inline void QuadraticQuadrilateral::print(PointerCollection &pointers) {
  auto &Logger = pointers.getSPDLogger();

  Logger.debug("Quadratic Quadrilateral Face id:  {:>12}",this->id);
  Logger.debug("Vertices:  {} {}",fmt::join(verts," "), fmt::join(verts_quad," "));
...
void QuadraticQuadrilateral::H1SolutionToParaview(
  ...
  for (auto i = 4; i < 9; ++i) {
    ...
    auto &V = pointers.getGeometryData()->getVertex(this->verts_quad[i - 4]);
...
auto QuadraticQuadrilateral::getGeometryShapes(PointerCollection &pointers,
    ...
    -> H1Shapes {
  H1Shapes shapes;
  shapes.shapes.resize(9);
  shapes.shapeDeriv.resize(2, 9);
