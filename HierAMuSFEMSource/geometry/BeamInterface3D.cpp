...
auto BeamInterface3D::getIntegrationPoints(PointerCollection &pointers, indexType elementId)
-> IntegrationPoints {
...
void BeamInterface3D::computeWarpingShapes(PointerCollection &pointers) {
  ...
  for (auto i : GP) {
    ...
    auto h1shapes = face->getH1Shapes(pointers, m_warpOrder, faceIntegration);
...
auto BeamInterface3D::getCoordinates(PointerCollection &pointers,
  ...
  coor = Face.getCoordinates(pointers, faceInt);
...
void BeamInterface3D::computeGeometry(PointerCollection &pointers) {
  ...
  x3 = face->getVertex(pointers, 3)->getCoordinates();
...
void BeamInterface3D::setH1Shapes(PointerCollection &pointers, indexType meshId,
  ...
  for (auto i : this->m_faces) {
    ...
    face->setH1Shapes(pointers, meshId, order, type);
  }
}
...
auto BeamInterface3D::getLocalCoordinate(PointerCollection &pointers,
  ...
  auto face = pointers.getGeometryData()->getFace(currFaceNumber);
  ...
  Types::Vector3<prec> faceCoordinate =
      face->getCoordinates(pointers, faceIntegrationPoint);
