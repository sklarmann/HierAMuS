...
namespace HierAMuS {
	...
	public:
		...
		void setInitialValues(indexType numberOfEquations, indexType numberOfActiveEquations) override;
		...
		Eigen::Matrix<prec, Eigen::Dynamic, 1> getSolution(
            std::vector<DegreeOfFreedom *> &Dofs) override;
		Eigen::Matrix<prec, Eigen::Dynamic, 1> getVelocity(
			std::vector<DegreeOfFreedom*> Dofs) override;
		Eigen::Matrix<prec, Eigen::Dynamic, 1> getAcceleration(
			std::vector<DegreeOfFreedom*> Dofs) override;
