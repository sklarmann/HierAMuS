...
class geometryObjects:
    ...
    def requestVertex(self,x,y,z):
        num = self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.Vertex);
    ...
    def requestLinearEdgeGeo(self,vertexList):
        ...
        number = self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.LinearEdge)
    ...
    def requestQuadraticEdge(self,vertexList):
        number = self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.QuadraticEdge)
        edge = self.geo.getEdge(number)
        edge.setVerts(self.geo,vertexList)
        return number
    ...
    def requestQuadrilateralFace(self,vertList,edgeList=None):
        ...
        number = self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.LinearQuadrilateral)
    ...
    def requestQuadraticQuadrilateralFace(self,vertList,edgeList=None):
        number = self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.QuadraticQuadrilateral)
        face = self.geo.getFace(number)
        face.setVerts(self.geo,vertList)
        if edgeList is not None:
            face.setEdges(edgeList)
    ...
    def requestTriangleFace(self,vertList,edgeList=None):
        ...
        number = self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.LinearTriangle)
        face = self.geo.getFace(number)
        face.setVerts(self.geo,vertList)
        if edgeList is not None:
            face.setEdges(edgeList)
    ...
    def requestLinearBrick(self,vertList,edgeList=None,faceList=None):
        number = self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.LinearBrick)
    ...
    def requestScaled2DFace(self,vertList,edgeList=None):
        number = self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.ScaledBoundary2D)
        face = self.geo.getFace(number)
        face.setVerts(self.geo,vertList)
        if edgeList is not None:
            face.setEdges(edgeList)
    ...
    def addVertex(self,num,x,y,z):
        self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.Vertex, num);
        vert = self.geo.getVertex(num)
        vert.setCoordinates(x,y,z)

    def addLinearEdgeGeo(self,number,vertexList):
        print("Deprecated function addLinearEdgeGeo, will be renamed to addLinearEdge")
        self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.LinearEdge,number)
        edge = self.geo.getEdge(number)
        edge.setVerts(self.geo,vertexList)
    ...
    def addQuadraticEdge(self,number,vertexList):
        self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.QuadraticEdge,number)
        edge = self.geo.getEdge(number)
        edge.setVerts(self.geo,vertexList)
    ...
    def addQuadrilateralFace(self,number,vertList,edgeList = None):
        ...
        self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.LinearQuadrilateral,number)
        face = self.geo.getFace(number)
        face.setVerts(self.geo,vertList)
        if edgeList is not None:
            face.setEdges(edgeList)
    ...
    def addQuadraticQuadrilateralFace(self,number,vertList,edgeList = None):
        self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.QuadraticQuadrilateral,number)
        face = self.geo.getFace(number)
        face.setVerts(self.geo,vertList)
        if edgeList is not None:
            face.setEdges(edgeList)
    ...
    def addTriangleFace(self,number,vertList,edgeList = None):
        ...
        self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.LinearTriangle,number)
        face = self.geo.getFace(number)
        face.setVerts(self.geo,vertList)
        if edgeList is not None:
            face.setEdges(edgeList)
    ...
    def addLinearBrick(self,number,vertList,edgeList=None,faceList=None):
        self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.LinearBrick,number)
        vol = self.geo.getVolume(number)
        vol.setVerts(self.geo,vertList)
        if edgeList is not None:
            vol.setEdges(edgeList)
    ...
    def addScaled2DFace(self,number,vertList,edgeList=None):
        self.geo.requestNewGeometryObject(HierAMuSPyFEM.Geometry.GeometryTypes.ScaledBoundary2D,number)
        face = self.geo.getFace(number)
        face.setVerts(self.geo,vertList)
        if edgeList is not None:
            face.setEdges(edgeList)
