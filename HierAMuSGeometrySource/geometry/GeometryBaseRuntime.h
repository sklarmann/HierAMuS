 ...
 *
 */
class GeometryBaseRuntime {

public:
  GeometryBaseRuntime(GeometryBaseData &base_element);
  virtual ~GeometryBaseRuntime();

  GeometryBaseRuntime(GeometryBaseRuntime &other);
  GeometryBaseRuntime(GeometryBaseRuntime &&other);
  virtual GeometryBaseRuntime &operator=(GeometryBaseRuntime &other);
  virtual GeometryBaseRuntime &operator=(GeometryBaseRuntime &&other);
   ...
   */
  virtual void
  setBoundaryCondition(indexType meshId, indexType order,
   ...
   */
  void setNodeSet(indexType meshID, indexType numberOfNodes, NodeTypes type);
   ...
   */
  auto checkSetWithMeshId(indexType meshId) -> bool;
  ...
  // Output Information
  virtual void print(spdlog::logger &Log) = 0;
  void printEqInfo(spdlog::logger &Log);

  virtual auto getIntegrationPoints(indexType elementId)
      -> IntegrationPoints; // move into specialized classes

  void updateNodes(std::map<indexType, NodeSetNodeList> &Nodes);
  void updateNodes(std::map<indexType, NodeSetNodeList> &&Nodes);

protected:
  std::map<indexType, NodeSetNodeList> m_Nodes;
  GeometryBaseData *m_Data_Element;
