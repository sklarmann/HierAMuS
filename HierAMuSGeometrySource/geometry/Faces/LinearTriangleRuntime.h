...
class LinearTriangleData;
class EdgesRuntime;

class LinearTriangleRuntime
    : public FacesRuntimeDataInterface<3, 3, LinearTriangleData,
                                       LinearTriangleRuntime>,
      public FacesH1Interface,
      public FacesHDivInterface {

public:
  LinearTriangleRuntime(GeometryData &geoData,
                        LinearTriangleData &base_element);
  ~LinearTriangleRuntime() override;
   ...
   * have multiple H1 nodes with different mesh ids (e.g. for different
   * solutions like displacements and rotations).
   *
   * @param pointers, object containing the pointers to global data.
  ...
  void getHDivShapes(indexType order, Types::Matrix2X<prec> &shape,
                     Types::VectorX<prec> &dshape, prec xi, prec eta) override;
